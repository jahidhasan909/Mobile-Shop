...
const MotorolaBuy = ({ motorola }) => {
    ...
    const handleQuantity = () => {
        const updateQuantity = motorolaCart.map(phone =>
            phone.id === motorola.id ? {
                ...phone, quantity: (phone.quantity || 1) + 1
            } : phone
        )
        setMotorolaCart(updateQuantity)
    }
    const handleQuantityMi = () => {
        const updateQuantity = motorolaCart.map(phone =>
            phone.id === motorola.id && phone.quantity > 1 ? {
        ...
        <div className='bg-white px-5 flex items-center rounded-md shadow-md justify-between'>
            ...
            <div className='flex flex-col items-center gap-2'>
                ...
                <div className='flex items-center gap-2'>
                    ...
                    <p className='bg-neutral-300 px-2 rounded-md'>{motorola.quantity || 1}</p>
